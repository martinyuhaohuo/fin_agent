...
def generate_data():
    ...
    outlier_indices = np.random.choice(np.where(~missing_mask)[0], 20, replace=False)
...
def process_pipeline():
    ...
    rolling_med = series.rolling(window=20, center=True).median()
    rolling_mad = series.rolling(window=20, center=True).apply(lambda x: np.median(np.abs(x - np.median(x))))
